...
export class BatchManagentComponent {
  ...
  batchResource = resource({
    ...
    loader: ({ params }) => {
      return fetch(`${this.apiUrl}/search?term=${encodeURIComponent(params.batchId)}&status=`).then(
        (res) => {
  ...
  realtimeAbv = computed(() => {
    ...
    if (!data) return 0;
    const og = data.densidadInicial || 1.09;
  ...
  async finalizeFiscalBatch(event: Event): Promise<void> {
    event.preventDefault();
    alert(
      `Batch successfully locked and linked to Tax Stamps: ${this.stampStart()} to ${this.stampEnd()}`,
    );
